...
<li style="visibility:hidden">Hidden field for space only for space some sme some </li>
  ...
  <li><a style="color:yellow" href="viewfiles.php"><img src="images/material.png" width="19.97px" height="14px" >  Files</a></li>
...
<div class="subnavmat"><a class="upld" href="uploadfile.php">&#8226 Upload</a><a class="viewall" href="#">&#8226  View all</a></div>
...
<option  value="viewfiles.php?c=tt">Time table</option>
<option  value="viewfiles.php?c=bo">E-books</option>
<option value="viewfiles.php?c=sm">Study materials</option>
<option value="viewfiles.php?c=zp">Zip files</option>
<option value="viewfiles.php?c=ot">Others</option>
</select>

<select id="sortby" name="sortby" class="category" onChange="window.location.href=this.value">
<option selected="selected"  value="">Sort by</option>
<option  value="viewfiles.php?s=nf&c=<?php  if(isset($_GET['c'])) echo $_GET['c']  ?>">Newest first</option>
<option  value="viewfiles.php?s=of&c=<?php  if(isset($_GET['c'])) echo $_GET['c']  ?>">Oldest first</option>
...
if(empty($sortby))
$query="SELECT * FROM materials WHERE matname LIKE '%$searitem%' ORDER BY datetime DESC LIMIT $no_of_rows";



if(isset($sortby)&&isset($cat)){
if(($sortby=="nf"))
$query="SELECT * FROM materials WHERE matname LIKE '%$searitem%' AND category LIKE '%$cat%' ORDER BY datetime DESC LIMIT $no_of_rows";

if($sortby=="of")
$query="SELECT * FROM materials WHERE matname LIKE '%$searitem%' AND category LIKE '%$cat%' ORDER BY datetime ASC LIMIT $no_of_rows";
}
...
echo '<p class="friend_name">'.$row['matname'].'<br>
<br><span style="font-weight:bold;opacity:.7">'.$row2['uname'].',&nbsp'.$row2['classname'].'
</span></p>';

echo '<div class="send_reque">';
echo '<a href="uploadedfiles/'.$row['matfile'].'" download><img src="images/dwnldicon.png" width="10.8px" height="14px" > Download</a>';
echo '</div>';
echo '</div>';
echo '<hr>';
...
<a class="show_more" href="viewfiles.php?rowno='.($no_of_rows+20).'&s='.$sortby.'&c='.$cat.'">show more</a></div>';
